...
***NOTES: 
need to switch datasets because invasion has empty samples
code: 
1) plot stats
2) 

```{r}
inv_div$index <- factor(inv_div$index, 
                        levels = c('N', 
                                   'S', 'beta_S', 
                                   'S_C', 'beta_S_C',
                                   'S_n', 'beta_S_n',
                                   'S_PIE', 'beta_S_PIE'))
ggplot(inv_div, aes(x = group, y = value)) +
  geom_boxplot(aes(col = scale)) + 
