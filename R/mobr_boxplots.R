...
#' 
#' The sampled matrices are either bootstrap or leave-one-out samples. The
#' bootstrap samples represent a random set of the rows sampled with replacement
#' from the original matrix. The leave-one-out samples are generated by removing
#' each sample one at a time from the original matrix. 
#' 
#' 
#' @inherit calc_comm_div 
#' @param algo can be either 'boot' or 'loo' for bootstrap or leave-one-out 
#' methods respectively. 
#' @param n_boot how to many boot strapped samples to create, defaults to 199.
#'
...
#' data(tank)
#' get_samples(tank)
get_samples <- function(abund_mat, algo = 'boot', n_boot = 199) {
...
#' and the confidence interval for each diversity index of interest. 
#' 
#' The measure of central tendency can be the median (default) or mean, and the
#' range of the confidence interval can be specified as well but defaults to a 95% 
#' confidence interval.  
#' 
