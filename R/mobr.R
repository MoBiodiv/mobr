  ...
  for (i in 1:length(groups)){
    ...
    if (i == 1)
      plot(as.numeric(as.character(dat_group$sample_plot)), as.numeric(as.character(dat_group$impl_S)), lwd = 2, type = 'l',
           xlab = 'N samples', ylab = 'Rarefied S', col = cols[i], ylim = c(0, max(as.numeric(as.character(mobr$sample_rare$impl_S)))),
  ...
  for (i in 1:length(groups)){
    ...
    if (i == 1)
      plot(as.numeric(as.character(dat_group$sample_plot)), as.numeric(as.character(dat_group$expl_S)), lwd = 2, type = 'l',
           xlab = 'N samples', ylab = 'Rarefied S', col = cols[i],ylim = c(0, max(as.numeric(as.character(mobr$sample_rare$expl_S)))),
...
# New input: swap - if swap is FALSE, S_samp_rare is generated on the input comm_group
#   if swap is TRUE, S_swap_rare is generated by Dan's new function avg_swap_rare()
# Output: a two-column data frame, with sample size (effort)
#   and deltaS (effect of N)
effect_of_N = function(comm_group, ref_dens, effort, swap = FALSE){
  group_sad = colSums(comm_group)
  if (swap == FALSE)
    S_samp_rare = as.numeric(rarefaction(comm_group, 'samp'))
  else
    S_samp_rare = avg_swap_rare(comm_group, rep(1, nrow(comm_group)), nperm = 100)
  ...
  # check tests
    ...
    if ('sampl' %in% approved_tests | 'spat' %in% approved_tests){
      ...
      for (group in group_levels){
        ...
        impl_S = avg_swap_rare(comm_group, rep(group, nrow(comm_group)), nperm = 100)
    ...
    if ('sampl' %in% approved_tests){
      ...
      if (type == 'continuous'){
        ...
        for (i in 1:length(group_levels)){
          ...
          group_effect_N = effect_of_N(comm_group, ref_dens, ind_sample_size, swap = T)
          if (i == 1)
        ...
        for (group in group_levels){
          if (as.character(group) != as.character(ref_group)){
            ...
            effect_N_ref = effect_of_N(comm$comm[which(as.character(env_data) == as.character(ref_group)), ], 
                                       ref_dens, ind_sample_size, swap = T)
            effect_N_group = effect_of_N(comm$comm[which(env_data == group), ], 
                                         ref_dens, ind_sample_size, swap = T)
