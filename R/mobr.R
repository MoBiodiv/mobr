...
deltaS_N = function(comm_group, ref_dens, inds){
   ...
   # TODO: do we really want to extrapolate?
   ...
   for(i in seq_along(group_levels)) {
      ...
      if (swap == 'noagg') {
         ...
         tmp_comm = t(sapply(plot_ids, function(x)
            table(factor(c(sp_draws[[x]], 1:length(meta_sad)))) - 1 ))
      }
      else
         stop('The argument swap must be either "noagg" or "swapN"')
      # The following lines are necessary as tmp_comm can have more columns than comm_group_perm
